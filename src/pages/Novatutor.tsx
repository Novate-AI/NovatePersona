      ...
      <div className="shrink-0 border-b flex items-center justify-between h-14 px-5" style={{ borderColor: 'var(--card-border)', background: 'var(--bg-main)' }}>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className={`h-8 px-2 md:px-2.5 rounded-lg flex items-center gap-1.5 transition-opacity shrink-0 text-secondary ${hasEnoughMessages && !feedbackLoading ? 'hover:text-primary hover:bg-black/5 dark:hover:bg-white/5' : feedbackLoading ? 'cursor-wait' : 'opacity-40 cursor-not-allowed'}`}
            title={hasEnoughMessages ? (feedbackLoading ? 'Generating feedback...' : 'Get feedback') : 'Get feedback (available after at least one exchange)'}
          >
            {feedbackLoading ? (
